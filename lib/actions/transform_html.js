...
TransformHtml.prototype.processFile = function(filepath) {
	...
	if (this.config.rewrite_urls) {
		...
		for (var i = 0; i < urls.length; i++) {
			assetpath = urls[i];
			if (!/^([\/\._a-zA-Z0-9])/.test(assetpath) || /^http(s)?:/.test(assetpath) || assetpath.indexOf('//') === 0) {
				//	skip absolute paths, protocol-relative paths, and anything that doesn't look like a relative path (eg # anchors)
				continue;
			} else if (assetpath.indexOf('/') === 0) {
				assetpath = path.join(this.workingDir, assetpath);
			} else if (assetpath.indexOf('.') === 0) {
				assetpath = path.resolve(path.dirname(filepath), assetpath);
			} else {
				assetpath = path.join(path.dirname(filepath), assetpath);
			}
			assetpath = this.fileChanges.getFinalPath(assetpath);
			assetpath = '/' + path.relative(this.workingDir, assetpath);
