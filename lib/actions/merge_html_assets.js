...
MergeHtmlAssets.prototype.processFile = function(filepath) {
	...
	flush = function() {
		...
		if (scripts.length > 1) {
			...
			filesetKeys.push(self.registerFileset.apply(self, scripts, filepath));
		}
		if (scripts.length > 0) {
			scripts = [];
		}
		if (styles.length > 1) {
			...
			filesetKeys.push(self.registerFileset.apply(self, styles, filepath));
		}
		if (styles.length > 0) {
			styles = [];
		}
	...
	parser = new htmlparser.Parser({
		onopentag: function(tagname, attrs) {
			...
			if (tagname === 'script') {
				if (current !== FLAGS.script) {
					flush();
				}
				current = FLAGS.script;
				if (!('src' in attrs)) {
					flush();
				} else if (!Url.resolveFilePath(attrs['src'], self.fileChanges, filepath, self.workingDir)) {
					//	does not reference an existing local file, skip
					//self.logger.info({en: 'no merge', path: attrs['src']});
					flush();
				} else {
					isOpenScript = true;
					...
					&& attrs['rel'].indexOf('stylesheet') < 0)
				{
					//	not a stylesheet
					flush();
				} else if (!Url.resolveFilePath(attrs['href'], self.fileChanges, filepath, self.workingDir)) {
					//	does not reference an existing local file, skip
					//self.logger.info({en: 'no merge', path: attrs['href']});
					flush();
...
MergeHtmlAssets.prototype.mergeFiles = function(files, filepath) {
	var mergeContent,
		type,
		mergeName,
		mergePath,
	...
	for (var i = 0; i < files.length; i++) {
		srcPath = Url.resolveFilePath(files[i], this.fileChanges, filepath, this.workingDir);

		if (!srcPath || !path.existsSync(srcPath)) {
			this.logger.emergency({
				ja: '"{{path}}" ファイルは見つかりません。そのパスにファイルがあることを確認してください。',
				en: 'File "{{path}}" not found. Please check that the path exists.',
				path: srcPath || '(none)',
				src: files[i],
				files: files
			});
...
MergeHtmlAssets.prototype.registerFileset = function(type, filepath) {
	var key,
		files,
		name,
		mergedName;
	...
	*/
	mergedName = this.mergeFiles(files, filepath);
