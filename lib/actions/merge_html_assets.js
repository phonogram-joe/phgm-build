...
MergeHtmlAssets.prototype.init = function(app) {
	...
	if (!('script_dir' in this.config)) {
		console.error(
			'merge_html_assetsを利用する場合は、script_dirの指定が必要です。', 
			'Must specify a script_dir to use merge_html_assets.'
		);
		process.exit(1);
	}
	if (!('style_dir' in this.config)) {
		console.error(
			'merge_html_assetsを利用する場合は、style_dirの指定が必要です。', 
			'Must specify a style_dir to use merge_html_assets.'
		);
		process.exit(1);
	}
	...
	if (this.config.script_name.indexOf('$HASH$') < 0 || this.config.style_name.indexOf('$HASH$') < 0) {
		console.error(
			'merge_html_assetsを利用する場合は、script_nameとstyle_nameには「$HASH$」を入れなければなりません。', 
			'Must put "$HASH" inside of script_name and style_name to use merge_html_assets.'
		);
		process.exit(1);
	}
	...
	if (this.config.hash_length < 0) {
		console.error(
			'merge_html_assetsのhash_lengthは１以上で指定してください。',
			'Please specify a value greater than 1 for merge_html_assets\'s hash_length.'
		);
		process.exit(1);
	}
}
