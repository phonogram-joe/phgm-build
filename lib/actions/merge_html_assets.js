...
MergeHtmlAssets.prototype.processFile = function(filepath) {
	...
	parser = new htmlparser.Parser({
		onopentag: function(tagname, attrs) {
			var assetpath;
			if (self.separator_class && 'class' in attrs && (new RegExp('\\b' + self.separator_class + '\\b')).test(attrs['class'])) {
				flush();
			}
			if (tagname === 'script') {
				if ('src' in attrs 
	...
	for (filesetIndex = 0; filesetIndex < filesetKeys.length; filesetIndex++) {
		...
		for (fileIndex = 0; fileIndex < files.length; fileIndex++) {
			if (type === FLAGS.script) {
				...
				content = content.replace(new RegExp('((\n|\r|\r\n)\s*)?<link[^>]*' + files[fileIndex] + '[^>]*>'), fileIndex === 0 ? '<link type="stylesheet" href="/' + mergePath + '" />' : '');
			}	
		}
	}
